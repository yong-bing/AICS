...
def power_difference_op(input_x,input_y,input_pow):
    with tf.Session() as sess:
        ...
        #---------------------------------------------------#
        plc_x = tf.placeholder(tf.float32, shape=input_x.shape, name='plc_x')
        plc_y = tf.placeholder(tf.float32, shape=input_y.shape, name='plc_y')
        plc_z= tf.placeholder(tf.float32, name='plc_z')
        out = tf.power_difference(plc_x, plc_y, plc_z)
        return sess.run(out, feed_dict = {plc_x:input_x, plc_y:input_y, plc_z:input_pow})
