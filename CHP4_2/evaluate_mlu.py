...
def ffwd(data_in, paths_out, model, device_t='', batch_size=1):
    ...
    # __________________
    #-------------------------------------------------------------#
    # TODO：如果 data_in 是保存输入图像的文件路径，则依次将该批次中输入图像文件路径下的 batch_size 张图像读入数组 X；如果 data_in 是已经读入图像并转化成数组形式的数据，则将该数组传递给 X
    if is_paths:
        # 读入第一张图像，由于 pb 模型的输入维度为 1 × 256 × 256 × 3, 因此需将输入图像的形状调整为 256 × 256，并传递给 img_shape；
        img_shape = get_img(data_in[0], (256, 256, 3)).shape
    else:
        # 直接获取图像的 shape 特征 img_shape
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(num_iters):
                ...
                #-------------------------------------------------------------#
                if is_paths:
                    # 依次将该批次中输入图像文件路径下的 batch_size 张图像读入数组 X
                    curr_batch_in = data_in[pos:pos+batch_size]
                    X = np.zeros(batch_shape, dtype=np.float32)
                    for j, path_in in enumerate(curr_batch_in):
                        img = get_img(path_in, img_shape)
                        X[j] = img
                else:
                    # 直接获取图像的 shape 特征 img_shape
